...
function route(config: AxiosRequestConfig): unknown {
  ...
  if (url.startsWith('/student/courses/')) { const course = mockCourses.find((item) => item._id === url.split('/').pop() || item.slug === url.split('/').pop()); if (!course) return { data: null }; return { data: { course, curriculum: course.curriculum, isEnrolled: true, enrollment: { progress: 78 } } }; }
