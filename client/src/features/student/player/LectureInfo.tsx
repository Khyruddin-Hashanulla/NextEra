      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {formatLectureDuration(lecture.duration) ? (
          <>
            ...
            <span className="text-sm text-muted-foreground tabular-nums">{formatLectureDuration(lecture.duration)}</span>
          </>
        ) : null}
        {typeof index === 'number' && typeof total === 'number' ? (
          <>
            <span className="h-1 w-1 rounded-full bg-muted-foreground/60" aria-hidden="true" />
            <Badge variant="secondary" className="normal-case">{index + 1} of {total}</Badge>
          </>
        ) : null}
      </div>
      {lecture.description ? <p className="text-sm leading-relaxed text-muted-foreground">{lecture.description}</p> : null}
    </div>
  );
}