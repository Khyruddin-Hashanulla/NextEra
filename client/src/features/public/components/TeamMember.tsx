...
export function TeamMember({ name, role, bio, avatar, twitter, linkedin, className }: TeamMemberProps) {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className={cn(
        'group rounded-2xl bg-background border border-border shadow-sm hover:shadow-md p-6 text-center transition-all duration-300',
        className
      )}
    >
        ...
        <div className="flex items-center justify-center gap-2 mt-4">
          {twitter && (
            <a
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-muted/50 text-muted-foreground/70 hover:bg-primary/10 hover:text-primary transition-all"
            >
