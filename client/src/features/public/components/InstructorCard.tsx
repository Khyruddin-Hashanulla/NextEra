...
export function InstructorCard({ instructor, className }: InstructorCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        'group block rounded-2xl bg-background border border-border shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-6 text-center',
        className
      )}
    >
      <div className="relative mx-auto w-20 h-20 mb-4">
        ...
        {instructor.rating && (
          <span className="absolute -bottom-1 -right-1 flex items-center gap-0.5 bg-background rounded-full px-2 py-0.5 shadow-sm border border-border text-xs font-semibold text-yellow-600">
      ...
      {instructor.specialties && instructor.specialties.length > 0 && (
        <div className="flex flex-wrap justify-center gap-1.5 mt-4">
          ...
          {instructor.specialties.length > 3 && (
            <span className="text-xs text-muted-foreground/70">+{instructor.specialties.length - 3} more</span>
          )}
        </div>
      )}
    </Link>
