...
export function FeatureCard({ icon, title, description, link, color = 'brand', className, highlighted }: FeatureCardProps) {
  ...
  return (
    ...
    >
      <div className={cn('inline-flex h-12 w-12 items-center justify-center rounded-xl mb-4 transition-colors', colors.bg, colors.text)}>
        {icon}
      </div>
      <h3 className="text-lg font-semibold text-foreground mb-2">{title}</h3>
      <p className="text-sm text-muted-foreground leading-relaxed mb-4">{description}</p>
      {link && (
        <Link to={link.href} className={cn('inline-flex items-center gap-1 text-sm font-medium transition-colors', colors.text, 'hover:underline')}>
