...
export function AssignmentsPage() {
  ...
  const columns = [
    {
      header: 'Assignment',
      accessor: (a: InstructorAssignmentItem) => (
        <div>
    ...
    { header: 'Max Marks', accessor: (a: InstructorAssignmentItem) => a.assignment?.totalMarks ?? 100 },
    {
      header: 'Due Date',
      accessor: (a: InstructorAssignmentItem) =>
        a.assignment?.dueDate ? new Date(a.assignment.dueDate).toLocaleDateString() : 'No deadline',
    },
    {
      header: 'Submissions',
      accessor: (a: InstructorAssignmentItem) => (
      ...
      accessor: (a: InstructorAssignmentItem) => (
        <Link to={ROUTES.INSTRUCTOR_ASSIGNMENT_SUBMISSIONS(a._id)}>
          <Button variant="outline" size="sm">
            View Submissions <ExternalLink className="ml-1 h-3 w-3" />
          </Button>
        </Link>
      ),
    },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm text-muted-foreground flex items-center gap-1">
          <ClipboardList className="h-4 w-4" /> Manage and grade student submissions
        </p>
      </div>
