...
import { buildCourseUpdatePayload, filterLectureData, getDefaultLectureData } from '@/features/instructor/components/course-editor/lectureData';
...
export function EditCoursePage() {
  ...
  const createSectionMutation = useMutation({
    ...
    onSuccess: () => { refresh(); addToast({ title: 'Section added', variant: 'success' }); },
  });

  const updateSectionMutation = useMutation({
    mutationFn: ({ sectionId, title }: { sectionId: string; title: string }) => instructorApi.updateSection(id || '', sectionId, { title }),
    onSuccess: () => { refresh(); addToast({ title: 'Section renamed', variant: 'success' }); },
  });

  const deleteSectionMutation = useMutation({
    mutationFn: (sectionId: string) => instructorApi.deleteSection(id || '', sectionId),
    onSuccess: () => { refresh(); addToast({ title: 'Section deleted', variant: 'success' }); setDeleteSectionId(null); },
  });

  const deleteLectureMutation = useMutation({
    mutationFn: (lectureId: string) => instructorApi.deleteLecture(id!, lectureId),
    onSuccess: () => { refresh(); addToast({ title: 'Lecture deleted', variant: 'success' }); setDeleteLectureId(null); },
  });

  const saveLectureMutation = useMutation({
    mutationFn: ({ lectureId, sectionId, data }: { lectureId?: string; sectionId?: string; data: any }) =>
      lectureId ? instructorApi.updateLecture(id || '', lectureId, data) : instructorApi.createLecture(id || '', sectionId!, data),
    onSuccess: () => {
      refresh();
  ...
  const renameLectureMutation = useMutation({
    mutationFn: ({ lectureId, title }: { lectureId: string; title: string }) => instructorApi.updateLecture(id || '', lectureId, { title }),
    onSuccess: () => { refresh(); addToast({ title: 'Lecture renamed', variant: 'success' }); },
  });

  const reorderSectionsMutation = useMutation({
    mutationFn: (sectionOrder: { sectionId: string; order: number }[]) => instructorApi.reorderSections(id || '', sectionOrder),
    onError: () => addToast({ title: 'Failed to reorder sections', variant: 'error' }),
  });

  const reorderLecturesMutation = useMutation({
    mutationFn: ({ sectionId, lectureOrder }: { sectionId: string; lectureOrder: { lectureId: string; order: number }[] }) =>
      instructorApi.reorderLectures(id || '', sectionId, lectureOrder),
    onError: () => addToast({ title: 'Failed to reorder lectures', variant: 'error' }),
  });
  ...
  const moveLectureMutation = useMutation({
    ...
    onSuccess: () => { refresh(); addToast({ title: 'Lecture moved', variant: 'success' }); },
    onError: (error: any) => addToast({ title: error?.response?.data?.message || 'Failed to move lecture', variant: 'error' }),
  });
  ...
  return (
    <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <CurriculumTab
                ...
                onReorderLectures={(sectionId, order) => reorderLecturesMutation.mutate({ sectionId, lectureOrder: order })}
                onRenameLecture={(lectureId, title) => renameLectureMutation.mutate({ lectureId, title })}
                onMoveLecture={(lectureId, targetSectionId) => moveLectureMutation.mutate({ lectureId, targetSectionId })}
              />
            </CardContent>
          </Card>
        </TabsContent>
      ...
      {deleteLectureId && <ConfirmDialog open onClose={() => setDeleteLectureId(null)} onConfirm={() => deleteLectureMutation.mutate(deleteLectureId)} title="Delete Lecture" description="Are you sure you want to delete this lecture?" loading={deleteLectureMutation.isPending} />}
...
function CourseDetailsForm({ course, categories, catsLoading, onSave, saving, onDirtyChange, savedCount }: { course: any; categories: any[]; catsLoading: boolean; onSave: (form: any) => void; saving: boolean; onDirtyChange: (dirty: boolean) => void; savedCount: number }) {
  ...
  useEffect(() => { snapshotRef.current = JSON.stringify(form); }, [savedCount]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <FileUploader
          ...
          value={form.thumbnail?.url ? { url: form.thumbnail.url, publicId: form.thumbnail.publicId, name: 'Course thumbnail' } : null}
          onChange={(file) => setForm({ ...form, thumbnail: file ? { url: file.url, publicId: file.publicId } : { url: '', publicId: '' } })}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label htmlFor="course-language" className="text-sm font-medium">Language</label>
          <Input id="course-language" value={form.language} onChange={(e) => setForm({ ...form, language: e.target.value })} />
        </div>
        <div className="space-y-2">
          <label htmlFor="course-badge" className="text-sm font-medium">Badge</label>
          <Input id="course-badge" value={form.badge || ''} onChange={(e) => setForm({ ...form, badge: e.target.value })} placeholder="e.g. Bestseller" />
        </div>
      </div>
      <div className="space-y-2">
        <label htmlFor="course-short-desc" className="text-sm font-medium">Short Description</label>
        <Textarea id="course-short-desc" value={form.shortDescription || ''} onChange={(e) => setForm({ ...form, shortDescription: e.target.value })} rows={2} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-desc" className="text-sm font-medium">Full Description</label>
        <Textarea id="course-desc" value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={6} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-welcome" className="text-sm font-medium">Welcome Message</label>
        <Textarea id="course-welcome" value={form.welcomeMessage || ''} onChange={(e) => setForm({ ...form, welcomeMessage: e.target.value })} rows={2} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-congrats" className="text-sm font-medium">Congratulation Message</label>
        <Textarea id="course-congrats" value={form.congratulationMessage || ''} onChange={(e) => setForm({ ...form, congratulationMessage: e.target.value })} rows={2} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-prereq" className="text-sm font-medium">Prerequisites</label>
        <Textarea id="course-prereq" value={form.prerequisites || ''} onChange={(e) => setForm({ ...form, prerequisites: e.target.value })} rows={2} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-benefits" className="text-sm font-medium">Benefits</label>
        <Textarea id="course-benefits" value={form.benefits || ''} onChange={(e) => setForm({ ...form, benefits: e.target.value })} rows={2} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-wyl" className="text-sm font-medium">What You Will Learn (one per line)</label>
        <Textarea id="course-wyl" value={Array.isArray(form.whatYouWillLearn) ? form.whatYouWillLearn.join('\n') : ''}
          onChange={(e) => setForm({ ...form, whatYouWillLearn: e.target.value.split('\n').filter(Boolean) })} rows={4} />
      </div>
      <div className="space-y-2">
        <label htmlFor="course-tags" className="text-sm font-medium">Tags (comma-separated)</label>
        <Input id="course-tags" value={Array.isArray(form.tags) ? form.tags.join(', ') : ''}
...
function PricingForm({ course, onSave, saving, onDirtyChange, savedCount }: { course: any; onSave: (form: any) => void; saving: boolean; onDirtyChange: (dirty: boolean) => void; savedCount: number }) {
  const [form, setForm] = useState<any>(() => ({
    ...course,
    pricing: course.pricing || { originalPrice: 0, discountPercent: 0, hasDiscount: false, gstPercent: 0, gstInclusive: true },
    certificateSettings: course.certificateSettings || { enabled: true, template: 'default', issueAutomatically: true, passingCriteria: 'completion', minimumQuizScore: 60 },
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <select id="course-type" value={form.courseType} onChange={(e) => setForm({ ...form, courseType: e.target.value })}
          className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
      ...
      <div className="space-y-2">
        ...
        <select id="course-visibility" value={form.visibility} onChange={(e) => setForm({ ...form, visibility: e.target.value })}
          className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input id="course-price" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} min={0} />
        </div>
        <div className="space-y-2">
          <label htmlFor="course-orig-price" className="text-sm font-medium">Original Price (for discount)</label>
          <Input id="course-orig-price" type="number" value={form.pricing?.originalPrice || 0}
            onChange={(e) => setForm({ ...form, pricing: { ...form.pricing, originalPrice: Number(e.target.value), hasDiscount: Number(e.target.value) > form.price } })} min={0} />
        </div>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label htmlFor="course-gst" className="text-sm font-medium">GST Percent</label>
          <Input id="course-gst" type="number" value={form.pricing?.gstPercent || 0}
            onChange={(e) => setForm({ ...form, pricing: { ...form.pricing, gstPercent: Number(e.target.value) } })} min={0} max={100} />
        </div>
        <div className="space-y-2">
          <label htmlFor="course-cert" className="text-sm font-medium">Certificate</label>
          <select id="course-cert" value={form.certificateSettings?.enabled ? 'yes' : 'no'}
            onChange={(e) => setForm({ ...form, certificateSettings: { ...form.certificateSettings, enabled: e.target.value === 'yes' } })}
            className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
      ...
      <Button onClick={() => onSave(form)} loading={saving}>Save Pricing</Button>
    </div>
  );
}
...
function SeoForm({ course, onSave, saving, onDirtyChange, savedCount }: { course: any; onSave: (form: any) => void; saving: boolean; onDirtyChange: (dirty: boolean) => void; savedCount: number }) {
  const [form, setForm] = useState<any>(() => ({
    ...course,
    pricing: course.pricing || { originalPrice: 0, discountPercent: 0, hasDiscount: false, gstPercent: 0, gstInclusive: true },
    certificateSettings: course.certificateSettings || { enabled: true, template: 'default', issueAutomatically: true, passingCriteria: 'completion', minimumQuizScore: 60 },
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="seo-title" value={form.meta?.seoTitle || ''} onChange={(e) => setForm({ ...form, meta: { ...form.meta, seoTitle: e.target.value } })} />
      </div>
      <div className="space-y-2">
        <label htmlFor="seo-desc" className="text-sm font-medium">SEO Description</label>
        <Textarea id="seo-desc" value={form.meta?.seoDescription || ''} onChange={(e) => setForm({ ...form, meta: { ...form.meta, seoDescription: e.target.value } })} rows={3} />
      </div>
      <div className="space-y-2">
        <label htmlFor="seo-keywords" className="text-sm font-medium">SEO Keywords (comma-separated)</label>
        <Input id="seo-keywords" value={Array.isArray(form.meta?.seoKeywords) ? form.meta.seoKeywords.join(', ') : ''}
          onChange={(e) => setForm({ ...form, meta: { ...form.meta, seoKeywords: e.target.value.split(',').map((t: string) => t.trim()).filter(Boolean) } })} />
      </div>
      <Button onClick={() => onSave(form)} loading={saving}>Save SEO</Button>
    </div>
  );
}
