  ...
  const formBody = (
    <div className="space-y-4 pr-1">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="space-y-2">
          <label htmlFor="lecture-type" className="text-sm font-medium">Type</label>
          <select
            id="lecture-type"
            value={form.type}
        ...
        <div className="space-y-2">
          <label htmlFor="lecture-duration" className="text-sm font-medium">Duration (seconds)</label>
          <Input
            id="lecture-duration"
            type="number"
            value={form.duration}
      ...
      <div className="space-y-2">
        <label htmlFor="lecture-description" className="text-sm font-medium">Description</label>
        <Textarea
      ...
      {form.type === 'video' && (
        <div className="space-y-4 rounded-xl border p-4">
          ...
          <LectureVideoPanel videoSource={form.videoSource || { source: 'none', url: '', videoId: '' }} onChange={(videoSource) => set({ videoSource })} />
        </div>
      )}

      {form.type === 'article' && (
        <div className="space-y-4 rounded-xl border p-4">
          <LectureArticlePanel articleContent={form.articleContent || ''} onChange={(articleContent) => set({ articleContent })} />
        </div>
      )}
      ...
      {form.type === 'assignment' && (
        <div className="space-y-4 rounded-xl border p-4">
          <LectureAssignmentPanel
            assignment={form.assignment || { question: '', instructions: '', totalMarks: 100, passingMarks: 60, allowLateSubmission: false, lateSubmissionDays: 7, penaltyPercent: 10 }}
  ...
  if (isMobile) {
    return (
      <Sheet open onOpenChange={(open) => !open && onClose()}>
        <SheetContent side="bottom" className="max-h-[92vh] overflow-y-auto sm:hidden">
          <SheetHeader>
            <SheetTitle>{lecture._id ? 'Edit Lecture' : 'Add Lecture'}</SheetTitle>
            <SheetDescription>
              Configure the lecture content. Press Ctrl/Cmd+S to save.
            </SheetDescription>
          </SheetHeader>
          {formBody}
          {footer}
