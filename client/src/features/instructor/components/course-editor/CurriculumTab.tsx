...
function SectionCard({
  ...
  const [_lectureOrder, setLectureOrder] = useState<string[]>(section.lectures?.map((l: any) => l._id) || []);
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button onClick={onToggle} className="flex items-center gap-2 font-medium">
            ...
            {section.totalDuration > 0 && (
              <span className="text-xs text-muted-foreground">{Math.round(section.totalDuration / 60)}m</span>
            )}
          </button>
        </div>
      ...
      {expanded && (
        <div className="px-4 py-2">
          <LectureSortableList
            ...
            onOrderChange={(order) => {
              setLectureOrder(order);
              onReorderLectures(order.map((id, i) => ({ lectureId: id, order: i })));
            }}
          />
          <button onClick={onAddLecture} className="mt-2 text-sm font-medium text-primary hover:underline">
            + Add lecture
          </button>
        </div>
      )}
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
      <SortableContext items={order} strategy={verticalListSortingStrategy}>
        <div className="divide-y">
          {orderedLectures.map((lecture: any) => (
            <LectureRow
              key={lecture._id}
              lecture={lecture}
              sections={sections}
...
function LectureRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center justify-between py-2 group ${isDragging ? 'opacity-50' : ''}`}
    >
      ...
      <div className="flex shrink-0 items-center gap-1">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            {sections
              .filter((s) => s._id !== lecture.section)
              .map((s) => (
...
export function CurriculumTab({
  ...
  useEffect(() => {
    setSectionOrder(curriculum.map((s) => s._id));
  }, [curriculum]);

  const orderedSections = sectionOrder
    .map((id) => curriculum.find((s) => s._id === id))
    .filter((s): s is any => !!s);
