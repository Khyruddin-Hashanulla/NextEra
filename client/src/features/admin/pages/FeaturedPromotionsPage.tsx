...
export function FeaturedPromotionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: ({ signal }) =>
      adminApi
        .listFeaturedPromotions({ page, limit: 10, status: statusFilter || undefined }, signal)
        .then((r) => r.data.data),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({ title: 'Promotion created', variant: 'success' });
      setOpen(false);
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-promotions'] });
      addToast({ title: 'Promotion updated', variant: 'success' });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-promotions'] });
      addToast({ title: 'Promotion deleted', variant: 'success' });
    },
