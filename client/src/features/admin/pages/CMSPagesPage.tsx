...
export function CMSPagesPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-cms-pages'] });
      addToast({ title: 'Page updated', variant: 'success' });
      setDialogOpen(false);
    },
  });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card key={page._id}>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${page.published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}
                  >
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-3 line-clamp-2">
                  {page.metaDescription || page.content?.slice(0, 100)}
                </p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => openEdit(page)}>
                    <Edit3 className="mr-1 h-3 w-3" /> Edit
                  </Button>
                  <Button variant="outline" size="sm" className="text-red-600" onClick={() => setDeleteId(page._id)}>
          ...
          {pages.length === 0 && (
            <p className="text-center text-muted-foreground py-8 col-span-full">No CMS pages yet</p>
          )}
        </div>
      )}
