...
export function WithdrawRequestsPage() {
  ...
  const columns = [
    ...
    { header: 'Status', accessor: (p: any) => (
      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${p.status === 'completed' ? 'bg-green-100 text-green-700' : p.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : p.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>{p.status}</span>
    ) },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <Button variant={statusFilter === 'completed' ? 'default' : 'outline'} size="sm" onClick={() => { setStatusFilter('completed'); setPage(1); }}>Completed</Button>
      </div>
      <DataTable columns={columns} data={payouts} isLoading={isLoading} pagination={pagination} page={page} onPageChange={setPage} />
    </div>
  );
}
