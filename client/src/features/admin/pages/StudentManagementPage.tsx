...
export function StudentManagementPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({ title: 'Student status updated', variant: 'success' });
  ...
  const columns = [
        ...
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-muted overflow-hidden flex-shrink-0">
            {s.avatar?.url ? (
              ...
              <div className="h-full w-full flex items-center justify-center text-xs font-medium">{s.name?.[0]}</div>
            )}
          </div>
          <div>
    ...
    { header: 'Enrollments', accessor: (s: any) => s.totalEnrollments || 0 },
    {
      ...
      accessor: (s: any) => (
        <span
          className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
        >
      ...
      accessor: (s: any) => (
        <div className="flex gap-1">
          <Button
            ...
            onClick={() => setConfirmAction({ id: s._id, action: s.isActive ? 'ban' : 'unban' })}
          >
          ...
          <Button
            ...
            onClick={() => setConfirmAction({ id: s._id, action: 'delete' })}
          >
  ...
  const confirmDesc =
    confirmAction?.action === 'ban'
      ...
      : confirmAction?.action === 'unban'
        ? 'Restore access for this student?'
