...
export function EmailTemplatesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (d: any) =>
      adminApi.createEmailTemplate({
        ...d,
        variables: d.variables ? d.variables.split(',').map((v: string) => v.trim()) : [],
      }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, d }: { id: string; d: any }) =>
      adminApi.updateEmailTemplate(id, {
        ...d,
        variables: d.variables ? d.variables.split(',').map((v: string) => v.trim()) : undefined,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-email-templates'] });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: '', slug: '', subject: '', body: '', variables: '', category: 'notification', isActive: true });
    setDialogOpen(true);
  };
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card key={t._id}>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${t.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}
                  >
              ...
              <CardContent>
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" className="text-red-600" onClick={() => setDeleteId(t._id)}>
          ...
          {templates.length === 0 && (
            <p className="text-center text-muted-foreground py-8 col-span-full">No email templates yet</p>
          )}
        </div>
      )}
