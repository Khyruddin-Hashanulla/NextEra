...
export function SupportTicketsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: ({ signal }) =>
      adminApi.listSupportTickets({ page, limit: 10, status: statusFilter || undefined }, signal),
  });
    ...
    { header: 'Assigned', accessor: (t: any) => t.assignedTo?.name || '-' },
    {
      ...
      accessor: (t: any) => (
        <div className="flex gap-1">
          ...
          {!t.assignedTo && (
            <Button variant="ghost" size="sm" onClick={() => assignMutation.mutate(t._id)}>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {detail && (
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[detail.status] || ''}`}>
                  {detail.status}
                </span>
              )}
            </DialogTitle>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                {detail.messages?.map((msg: any, i: number) => (
                  <div
                    key={i}
                    className={`rounded-lg p-3 ${msg.sender?._id === detail.user?._id ? 'bg-muted ml-8' : 'bg-primary/5 mr-8'}`}
                  >
              ...
              <div className="flex gap-2">
                <select
                  ...
                  value={detail.status}
                  onChange={(e) => statusMutation.mutate({ id: detail._id, status: e.target.value })}
                >
                ...
                {!detail.assignedTo && (
                  <Button variant="outline" size="sm" onClick={() => assignMutation.mutate(detail._id)}>
                    <UserCheck className="mr-1 h-3 w-3" /> Assign to me
                  </Button>
                )}
              </div>
              <div className="flex gap-2">
                ...
                />
                <Button
                  onClick={() => replyMutation.mutate({ id: detail._id, message: replyText })}
