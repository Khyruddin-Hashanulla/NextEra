...
export function SecurityLogsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: ({ signal }) =>
      adminApi.listSecurityLogs(
        { page, limit: 20, severity: severityFilter || undefined, event: eventFilter || undefined },
        signal
      ),
  });
  ...
  const columns = [
    ...
    {
      header: 'Event',
      accessor: (l: any) => <span className="text-sm font-medium">{l.event.replace(/_/g, ' ')}</span>,
    },
    {
      header: 'Severity',
      accessor: (l: any) => (
        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${severityColors[l.severity] || ''}`}>
          {l.severity}
        </span>
      ),
    },
    { header: 'IP', accessor: (l: any) => l.ip || '-' },
    {
      header: 'Date',
      accessor: (l: any) => (
        <span className="text-sm text-muted-foreground">{new Date(l.createdAt).toLocaleString()}</span>
      ),
    },
  ];
