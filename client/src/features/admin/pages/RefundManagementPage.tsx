...
export function RefundManagementPage() {
  ...
  const columns = [
    ...
    { header: 'Status', accessor: (r: any) => (
      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${r.status === 'approved' ? 'bg-green-100 text-green-700' : r.status === 'rejected' ? 'bg-red-100 text-red-700' : r.status === 'processed' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>{r.status}</span>
    ) },
    { header: 'Actions', accessor: (r: any) => (
      <div className="flex gap-1">
        <Button variant="ghost" size="sm" onClick={() => setDetailRefund(r)}>View</Button>
        {r.status === 'pending' && (
          <>
            <Button variant="ghost" size="sm" className="text-green-600" onClick={() => setProcessTarget({ id: r._id, action: 'approve' })}>Approve</Button>
            <Button variant="ghost" size="sm" className="text-red-600" onClick={() => setProcessTarget({ id: r._id, action: 'reject' })}>Reject</Button>
          </>
        )}
      </div>
    ) },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <Button variant={statusFilter === 'rejected' ? 'default' : 'outline'} size="sm" onClick={() => { setStatusFilter('rejected'); setPage(1); }}>Rejected</Button>
      </div>
      <DataTable columns={columns} data={refunds} isLoading={isLoading} pagination={pagination} page={page} onPageChange={setPage} />
        ...
        <DialogContent>
          ...
          {detailRefund && (
            <div className="space-y-3 text-sm">
              <div><strong>User:</strong> {detailRefund.user?.name} ({detailRefund.user?.email})</div>
              <div><strong>Amount:</strong> ₹{detailRefund.amount}</div>
              <div><strong>Reason:</strong> {detailRefund.reason}</div>
              <div><strong>Status:</strong> {detailRefund.status}</div>
              {detailRefund.adminNote && <div><strong>Admin Note:</strong> {detailRefund.adminNote}</div>}
            </div>
          )}
        </DialogContent>
      </Dialog>

      <Dialog open={!!processTarget} onOpenChange={() => { setProcessTarget(null); setAdminNote(''); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>{processTarget?.action === 'approve' ? 'Approve Refund' : 'Reject Refund'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Admin Note (optional)</Label><Input value={adminNote} onChange={(e) => setAdminNote(e.target.value)} /></div>
            <Button onClick={() => {
