...
import { Search, Trash2, RotateCcw, ExternalLink, Shield } from 'lucide-react';
...
export function CertificatesManagementPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: ({ signal }) => adminApi.listCertificates({ page, limit: 10, search: search || undefined, status: statusFilter === 'all' ? undefined : statusFilter }, signal),
  });

  const revokeMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason?: string }) => adminApi.revokeCertificate(id, reason),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-certificates'] }); addToast({ title: 'Certificate revoked', variant: 'success' }); setActionId(null); setRevokeReason(''); },
  });

  const restoreMutation = useMutation({
    mutationFn: (id: string) => adminApi.restoreCertificate(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-certificates'] }); addToast({ title: 'Certificate restored', variant: 'success' }); setActionId(null); },
  });
  ...
  const columns = [
    ...
    { header: 'Certificate ID', accessor: (c: CertificateItem) => <code className="rounded bg-muted px-1.5 py-0.5 text-xs">{c.certificateId}</code> },
    { header: 'Status', accessor: (c: CertificateItem) => (
      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${c.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        {c.status === 'active' ? 'Active' : 'Revoked'}
      </span>
    )},
    { header: 'Issued', accessor: (c: CertificateItem) => new Date(c.issuedAt).toLocaleDateString() },
    { header: 'Actions', accessor: (c: CertificateItem) => (
  ...
  const confirmDescription = actionType === 'revoke'
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        open={!!actionId}
        onOpenChange={() => { setActionId(null); setRevokeReason(''); }}
