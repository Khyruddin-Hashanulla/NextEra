...
export function ReviewsModerationPage() {
  ...
  const moderateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({ title: 'Review moderated', variant: 'success' });
  ...
  const columns = [
    ...
    {
      header: 'Review',
      accessor: (r: any) => <p className="max-w-[300px] truncate text-sm text-muted-foreground">{r.review}</p>,
    },
    {
      header: 'Status',
      accessor: (r: any) => (
        <span
          className={`rounded-full px-2 py-0.5 text-xs font-medium ${r.status === 'approved' ? 'bg-green-100 text-green-700' : r.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}
        >
          {r.status}
        </span>
      ),
      ...
      accessor: (r: any) => (
        <div className="flex gap-1">
          <Button
            ...
            onClick={() => setModerateTarget({ id: r._id, action: 'approved' })}
          >
            Approve
          </Button>
          <Button
            ...
            onClick={() => setModerateTarget({ id: r._id, action: 'rejected' })}
          >
            Reject
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        open={!!moderateTarget}
        onOpenChange={() => setModerateTarget(null)}
        onConfirm={() => moderateMutation.mutate({ id: moderateTarget!.id, status: moderateTarget!.action })}
