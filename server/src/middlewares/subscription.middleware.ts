...
export const requireSubscription = asyncHandler(async (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (!subscriptionPermissionService.isActive(planInfo)) {
    return next(createSubscriptionError('Pro', 'You need an active subscription plan. Upgrade to access this feature.'));
  }
  req.instructorPlan = planInfo;
  next();
});

export const requireFeaturePermission = (featureCheck: (info: import('../services/subscriptionPermission.service').InstructorPlanInfo) => boolean, featureName: string, upgradePlan: string) =>
  asyncHandler(async (req: Request, _res: Response, next: NextFunction) => {
    const planInfo = await subscriptionPermissionService.getInstructorPlanInfo(req.currentUser!.userId);
    if (!featureCheck(planInfo)) {
      return next(createSubscriptionError(upgradePlan, `${featureName} is available on the ${upgradePlan} plan. Upgrade to access this feature.`));
    }
    req.instructorPlan = planInfo;
    next();
  });

function createSubscriptionError(requiredPlan: string, message?: string) {
  const err: any = new Error(message || `${requiredPlan} subscription required. Please upgrade your plan.`);
  err.statusCode = 403;
