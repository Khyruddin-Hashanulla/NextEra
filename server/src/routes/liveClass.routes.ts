...
import {
  ...
  addRecording,
  deleteRecording,
  syncInstructorRecordings,
  getInstructorRecording,
  listStudentLiveClasses,
  joinLiveClass,
  leaveLiveClass,
  listStudentRecordings,
  incrementRecordingView,
...
router.post(
  ...
  validate(addRecordingSchema),
  addRecording
);
router.post(
  '/instructor/recordings/sync',
  authorize(ROLES.INSTRUCTOR, ROLES.ADMIN),
  validate(syncRecordingSchema),
  syncInstructorRecordings
);
router.get(
  '/instructor/recordings/:id',
  authorize(ROLES.INSTRUCTOR, ROLES.ADMIN),
...
router.post(
  '/',
  authorize(ROLES.INSTRUCTOR, ROLES.ADMIN),
  requireFeaturePermission((info) => subscriptionPermissionService.canScheduleLiveClass(info), 'Live classes', 'Pro'),
