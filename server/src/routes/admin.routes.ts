...
import { overrideGradeSchema, submissionsListQuerySchema } from '../validators/assignment.validator';
...
import { syncRecordingSchema, recordingParamsSchema, recordingsQuerySchema } from '../validators/liveClass.validator';
...
router.post('/blog', audit({ action: 'BLOG_CREATED', resourceType: 'Blog' }), validate(createBlogSchema), createBlog);
router.put(
  '/blog/:id',
...
router.put(
  ...
  audit({
    ...
    getPreviousData: previousDataLoader(PlatformSettings),
  }),
...
router.post(
  ...
  audit({
    ...
    resourceType: 'Payout',
    getPreviousData: async (_req: any) => {
      return undefined;
    },
  }),
  approvePayout
...
router.post(
  '/payouts/process-all',
  audit({ action: 'PAYOUT_PROCESSED', resourceType: 'Payout' }),
  processAllPendingPayouts
);
...
router.put(
  ...
  audit({
    ...
    getPreviousData: previousDataLoader(Subscription),
  }),
...
router.post('/faq', audit({ action: 'FAQ_CREATED', resourceType: 'Faq' }), validate(createFaqSchema), createFaq);
router.put(
  '/faq/:id',
...
router.put(
  ...
  audit({
    ...
    getPreviousData: previousDataLoader(EmailTemplate),
  }),
...
router.post('/backups', audit({ action: 'BACKUP_STARTED', resourceType: 'Backup' }), createBackup);
router.delete('/backups/:id', audit({ action: 'BACKUP_DELETED', resourceType: 'Backup' }), deleteBackup);
...
router.put(
  ...
  audit({
    ...
    getPreviousData: previousDataLoader(RolePermission),
  }),
...
router.put('/features/:key', audit({ action: 'FEATURE_TOGGLE_CHANGED', resourceType: 'FeatureToggle' }), updateFeature);
router.post('/features/seed', audit({ action: 'FEATURE_TOGGLE_CHANGED', resourceType: 'FeatureToggle' }), seedFeatures);
...
router.delete(
  ...
  validate(recordingParamsSchema, 'params'),
  deleteAdminRecording
);
