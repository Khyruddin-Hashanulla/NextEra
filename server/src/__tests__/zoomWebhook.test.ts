...
describe('verifyZoomWebhookSignature', () => {
  ...
  it('returns true for a valid signature with millisecond timestamp', () => {
    ...
    expect(
      verifyZoomWebhookSignature({ secret: SECRET, signature, timestamp, rawBody })
    ).toBe(true);
  });
  ...
  it('returns false when secret is empty', () => {
    ...
    const signature = sign(rawBody, timestamp);
    expect(
      verifyZoomWebhookSignature({ secret: '', signature, timestamp, rawBody })
    ).toBe(false);
  });

  it('returns false when signature or timestamp headers are missing', () => {
    expect(
      verifyZoomWebhookSignature({ secret: SECRET, signature: undefined, timestamp: undefined, rawBody })
    ).toBe(false);
  });
});
